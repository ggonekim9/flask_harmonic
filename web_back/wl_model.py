...
class wl_model():
    ...
    def t_q_res(flow_q, mean_water_level, tide_range):
        ...
        R = tide_range[0:-1]  # 潮差日期选择
        R = np.array(R)

        h = mean_water_level[1:]  # 水位日期选择
    ...
    def verification_f(mean_water_level, flow_q, tide_range, param):
        ...
        plt.plot(x1, y3, color='blue', label='q_t')
...
if __name__ == '__main__':
    ...
    path_wl = r'nanjing.xlsx'
    path_q = 'datong.xlsx'
    path_t = 'baozhen.xlsx'
    dataf = wl_model.read_waterl_Excel(path_wl)
    dataq = wl_model.read_q_Excel(path_q)
    datat = wl_model.read_tide_range(path_t)
    flow_q = wl_model.get_flow(dataq)  # 获取流量数据
    mean_water_level = wl_model.get_mean_water_level(dataf)  # 只考虑径流，获取平均水位数据
    tide_range = wl_model.get_tide_range(datat)  # 获取日平均潮差

    # wl_model.q_res(flow_q,mean_water_level)  # 考虑径流的回归分析

    param = []
    param = wl_model.t_q_res(flow_q,mean_water_level,tide_range) # 同时考虑径流和潮差的回归分析

    # wl_model.verification_f(mean_water_level,flow_q,tide_range,param)  # 预测模型验证
